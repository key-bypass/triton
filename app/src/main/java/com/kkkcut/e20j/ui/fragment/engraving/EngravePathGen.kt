...
object EngravePathGen {
    /* JADX WARN: Multi-variable type inference failed */ /* JADX WARN: Type inference failed for: r5v6 */
            ...
            if (i5 >= i6) {
                ...
                arrayList.add(
                    ...
                    StepBean(
                        ...
                        stepBean.z - 100,
                        "",
                        "C:5,X;C:5,Y;C:5,Z;SUP:1,8000",
                        false
                    )
                )
                val stepBean2 = arrayList[arrayList.size - 1]
                arrayList.add(
                    StepBean(
                        0,
                        stepBean2.x,
    ...
    fun detectKeyPosition(engraveParam: EngraveParam): List<StepBean> {
        val arrayList = ArrayList<StepBean>()
        arrayList.add(
            StepBean(
                "移动到夹具上方",
    ...
    fun detectCutterHigh(bitmap: Bitmap, engraveParam: EngraveParam): List<StepBean> {
        ...
        val f = clampOriginX.toFloat()
        val dcX = engraveParam.dcX.toFloat()
        val f2 = clampOriginY.toFloat()
        val dcY = engraveParam.dcY.toFloat()
        arrayList.add(
            StepBean(
                "移动到夹具上方",
                0,
                ((f - (300.0f / MachineData.dXScale)) + dcX).toInt(),
                ((f2 - (450.0f / MachineData.dYScale)) + dcY).toInt(),
                0,
                "8000,8000,3000",
                "C:5,X;C:5,Y;",
                false
        ...
        arrayList.add(StepBean("探测夹具上表面", 1, 0, 0, 3500, "4000,4000,3000", "U:Z"))
        arrayList.add(
            StepBean(
                "离开夹具上表面",
                0,
        ...
        arrayList.add(
            StepBean(
                "移动到切割区域中点",
                0,
                ((f - ((width + engraveParam.pictureOriginX) / MachineData.dXScale)) + dcX).toInt(),
                (f2 + ((height + engraveParam.pictureOriginY) / MachineData.dYScale) + dcY).toInt(),
                0,
                "8000,8000,3000",
                "C:5,X;C:5,Y;",
                false
        ...
        arrayList.add(StepBean("探测钥匙表面", 1, 0, 0, 180, "4000,4000,3000", "SL:Z,Z;"))
        arrayList.add(StepBean("离开钥匙表面", 0, 0, 0, -100, "8000,8000,3000", "S:5,Z;C:3,K,Z-5"))
        return arrayList
    }
    ...
    private fun addCutPoint(
        ...
        val pictureOriginX =
            ((clampOriginX - (((i * 10) + engraveParam.pictureOriginX) / MachineData.dXScale)) + dcX).toInt()
        val pictureOriginY =
            (clampOriginY + (((i2 * 10) + engraveParam.pictureOriginY) / MachineData.dYScale) + dcY).toInt()
