...
public class C2019e {
    ...
    public static void m130a(Bitmap bitmap, Context context, a aVar) {
        File file;
        try {
            file = File.createTempFile("feedback_", ".jpg", new File(C2038c.m196a().m205d(context)));
        } catch (IOException e) {
            e.printStackTrace();
            file = null;
        }
        if (file != null) {
            C2041f.m220d("PgyerSDK", "saveScreeenShot image path:" + file.toString());
        } else {
            C2041f.m220d("PgyerSDK", "saveScreeenShot image path: NULL");
        }
        try {
