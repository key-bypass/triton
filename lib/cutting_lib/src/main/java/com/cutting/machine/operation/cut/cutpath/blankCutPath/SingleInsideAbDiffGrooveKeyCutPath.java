...
public class SingleInsideAbDiffGrooveKeyCutPath extends KeyBlankCutPath {
    private static final float[] layer2 = {0.7f, 1.0f};
    private static final float[] layer3 = {0.4f, 0.8f, 1.0f};

    public SingleInsideAbDiffGrooveKeyCutPath(KeyAlignInfo keyAlignInfo, DataParam dataParam) {
    ...
    */
    public java.util.List<com.cutting.machine.bean.StepBean> getCutPathSteps() {
        /*
            Method dump skipped, instructions count: 754
            To view this dump change 'Code comments level' option to 'DEBUG'
        */
        throw new UnsupportedOperationException("Method not decompiled: com.cutting.machine.operation.cut.cutpath.blankCutPath.SingleInsideAbDiffGrooveKeyCutPath.getCutPathSteps():java.util.List");
    }

    private void getSecondarySideGroup(List<List<KeyPoint>> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            List<KeyPoint> list3 = list.get(i);
            int i2 = 0;
            while (i2 < list3.size() - 1) {
                KeyPoint keyPoint = list3.get(i2);
                i2++;
                KeyPoint keyPoint2 = list3.get(i2);
    ...
    public List<List<KeyPoint>> getEndPointsGroup() {
        ArrayList arrayList = new ArrayList();
        for (int i = 0; i < getKeyPointsGroup().size(); i++) {
            List<KeyPoint> group = this.getKeyPointsGroup().get(i);
            KeyPoint keyPoint = group.get(group.size() - 1);
            ArrayList arrayList2 = new ArrayList();
            int nose = getNose();
                    ...
                    if (i7 < i8) {
                        ...
                        while (i10 < list4.size()) {
                            ...
                            String sb = "右边第" +
                                    ...
                                    i11 +
                                    "点";
                            arrayList.add(StepBeanFactory.getCutStepBean(sb, keyX2MachineValue2, KeyY2MachineValue2, i6, Speed.get_Speed_CuttingIn(), "C:5,X;C:5,Y;C:5,Z;CUTSM;"));
                            if (i10 == list4.size() - 1) {
