...
public class SingleInsideGrooveKeyCutNewPath extends KeyBlankCutPath {
    ...
    */
    public java.util.List<com.cutting.machine.bean.StepBean> getCutPathSteps() {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.cutting.machine.operation.cut.cutpath.blankCutPath.SingleInsideGrooveKeyCutNewPath.getCutPathSteps():java.util.List");
    }
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            while (i2 < list2.size() - 1) {
                ...
                if (keyPoint2.getX() - keyPoint.getX() != 0) {
                    int r6 = (getGroove() - getCutterRadiusSize());
                    int groove = (int) (r6 * Math.tan(Math.atan(((keyPoint2.getY() - keyPoint.getY()) * 1.0f) / r6) / 2.0d));
    ...
    protected int[] splitCut(int i, int i2) {
        ...
        for (int i3 = 0; i3 < cutCount; i3++) {
            int r4 = getCutterSize();
            iArr[i3] = ((int) ((sideA(i2) ? i3 : r4 - i3) * f)) + i + r4;
        }
        return iArr;
    }
